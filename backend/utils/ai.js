...
const analyzeTicket = async (ticket) => {
  const supportAgent = createAgent({
    ...
    description:
      "Analyzes tickets mainly related to software development and assigns them smartly",
...
- Description: ${ticket.description}`);
  ...
  const result = response.output[0].content;
  // console.log("AI result", result);

  try {
    const match = result.match(/```json\s*([\s\S]*?)\s*```/i);
    const jsonString = match ? match[1] : result.trim();
